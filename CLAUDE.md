...
`npm test` runs the Node test runner via the existing ts-node dependency. It currently covers 133 tests across twenty-five script files, including dashboard statistics, projected progress, strength trends, exercise-performance history, muscle-group heatmaps, volume trends, session comparison, progress timelines, measurable personal goals, milestone achievements, session recap/recovery, live records, progression, profile privacy/discovery, relationship lists/suggestions, session sharing, exercise substitutions, the exercise catalog and training locations; it is included in `npm run verify`.
...
**Milestone achievements are immutable analytics events (ACH-01/DATA-02).** The shared catalog defines 25 thresholds across completed sessions, sets, external-load volume, current personal-record exercises and best streak days. `awardMilestoneAchievements` runs inside the account-scoped analytics writer and uses unique event keys, so finish retries and rebuilds do not duplicate `ACHIEVEMENT_UNLOCKED` or `STREAK_MILESTONE` events. `GET /achievements` reconciles a READY projection plus the bounded current-record count before reading; events recognized from pre-feature history use `sessionId = null`, `backfilled = true` and must never be presented as if an exact earned date were known. Keep the catalog finite unless the read limit and tests change together.
