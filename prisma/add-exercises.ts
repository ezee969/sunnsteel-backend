...
/**
 * Full exercise catalog, upserted by name so IDs and references are preserved
 * across runs. This is the idempotent seed used by `npm run db:seed` /
 * `db:add-exercises` and the Prisma migrate seed hook.
 *
 * To add exercises: append entries to `exercises` below, then run
 * `npx tsx prisma/add-exercises.ts` (or `npm run db:seed`).
 */
