...
const workoutsServiceMock = {
  ...
  getActiveSession: jest
    .fn()
    .mockResolvedValue({ id: 'sess-reuse', status: 'IN_PROGRESS' }),
};

const allowGuard: Partial<SupabaseJwtGuard> = {
  canActivate: (ctx: any) => {
    const req = ctx.switchToHttp().getRequest();
    req.user = { id: 'user-2', email: 'u2@example.com' };
    return true as any;
  },
};
...
describe('Workouts start reuse e2e (mocked service)', () => {
  ...
  it('second start returns reused flag', async () => {
    ...
    expect(first.body).toEqual(
      expect.objectContaining({ id: 'sess-reuse', reused: false }),
    );
    ...
    expect(second.body).toEqual(
      expect.objectContaining({ id: 'sess-reuse', reused: true }),
    );
