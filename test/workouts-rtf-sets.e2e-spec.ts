...
describe('Workout Planning RtF Variant Sets (e2e mocked)', () => {
	...
	const workoutsServiceMock = {
		...
		listSessions: jest.fn().mockResolvedValue({ items: [], nextCursor: undefined }),
	...
	it('verifies standard vs hypertrophy set counts in mocked planning output shape', async () => {
			...
			items: [
				{
					...
					status: 'IN_PROGRESS',
					planned: [
						{ routineExerciseId: 're-std', progressionScheme: 'PROGRAMMED_RTF', sets: [1,2,3,4,5].map(n => ({ setNumber: n })) },
						{ routineExerciseId: 're-hyp', progressionScheme: 'PROGRAMMED_RTF_HYPERTROPHY', sets: [1,2,3,4].map(n => ({ setNumber: n })) },
					],
				},
			],
		...
		const std = planned.find((p: any) => p.progressionScheme === 'PROGRAMMED_RTF')
		const hyp = planned.find((p: any) => p.progressionScheme === 'PROGRAMMED_RTF_HYPERTROPHY')
