...
describe('TM Adjustments on Forwarded Weeks (e2e)', () => {
  ...
  const mockUser = {
    id: 'ffwd-tm-user',
    email: `ffwd-tm-${Date.now()}@example.com`,
  };
  ...
  beforeAll(async () => {
    ...
    const u = await prisma.user.create({
      data: { email: mockUser.email, name: 'FFWD TM User' },
    });
    userId = u.id;
    mockUser.id = userId;
  ...
  it('Deload week: no TM adjustment even if AMRAP reps are high', async () => {
    ...
    const beforeEx = await prisma.routineExercise.findUnique({
      where: { id: rtfPlan.routineExerciseId },
    });
    ...
    const afterEx = await prisma.routineExercise.findUnique({
      where: { id: rtfPlan.routineExerciseId },
    });
