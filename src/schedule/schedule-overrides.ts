...
export interface MoveInput {
  ...
  others: ReadonlyArray<{ date: string; toDate: string | null }>;
}
 ...
 * date where the routine is already planned. The server does not know the
 * device's time zone, so "past" allows one day of slack; the app applies
 * the exact local rule.
 */
export function assertMovable({ date, toDate, now, routine, others }: MoveInput) {
  if (routine.scheduleMode !== 'WEEKLY') {
    throw new BadRequestException(
      'Only weekly routines have dated workouts to move',
    );
  }
  ...
  const earliest = addCalendarDays(now.toISOString().slice(0, 10), -1);
  if (date < earliest || toDate < earliest) {
    throw new BadRequestException('Past workouts cannot be moved');
  }
  const movedAway = others.some((o) => o.date === toDate && o.toDate);
  if (routine.trainingWeekdays.includes(weekdayOf(toDate)) && !movedAway) {
