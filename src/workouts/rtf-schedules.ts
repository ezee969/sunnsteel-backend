...
export const RTF_HYPERTROPHY_WITH_DELOADS: Array<
  RtfHypertrophyGoal | RtfDeload
> = [
...
export function buildRtfProgramSnapshot(
  withDeloads: boolean,
): RtfProgramSnapshot {
  const version = 1;
  const now = new Date().toISOString();
  const filterTraining = (arr: any[]) =>
    arr.filter((g) => !(g.isDeload === true));
  const stdSource = withDeloads
    ? RTF_STANDARD_WITH_DELOADS
    : filterTraining(RTF_STANDARD_WITH_DELOADS).map((g: any, idx: number) => ({
        ...g,
        week: idx + 1,
      }));
  const hypSource = withDeloads
    ? RTF_HYPERTROPHY_WITH_DELOADS
    : filterTraining(RTF_HYPERTROPHY_WITH_DELOADS).map(
        (g: any, idx: number) => ({ ...g, week: idx + 1 }),
      );
