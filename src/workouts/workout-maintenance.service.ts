...
export class WorkoutMaintenanceService {
	...
	constructor(private readonly config: ConfigService, private readonly db: DatabaseService) {
		this.timeoutHours = Number(this.config.get('WORKOUT_SESSION_TIMEOUT_HOURS') ?? 48)
		this.sweepIntervalMin = Number(this.config.get('WORKOUT_SESSION_SWEEP_INTERVAL_MIN') ?? 30)
	}
	...
	@Interval(60_000)
	async sweep() {
		const intervalMs = Math.max(5, Number(this.sweepIntervalMin || 30)) * 60_000
		const now = Date.now()
		...
		try {
			...
			if (result.count > 0) {
				this.logger.log(`Auto-aborted ${result.count} stale sessions (> ${this.timeoutHours}h)`) 
			}
		} catch (e) {
			this.logger.error('Failed sweeping stale workout sessions', e as any)
		}
	}
}
