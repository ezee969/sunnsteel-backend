...
@Injectable()
export class InMemoryRtfWeekGoalsCacheProvider implements IRtfWeekGoalsCacheProvider, OnModuleDestroy {
	...
	constructor() {
		...
		this.logger.log(`Initialized in-memory RtF cache TTL=${this.defaultTtlMs}ms`)
	}
  ...
  // --- Sync (deprecated) ---
	...
	invalidateRoutine(routineId: string): void {
		...
		if (removed > 0) this.logger.log(`Invalidated ${removed} keys for routine ${routineId}`)
		if (removed > 0) this.metrics.invalidations++
	}

	// Metrics snapshot used by telemetry service
	getMetrics() {
		return { driver: 'memory', ...this.metrics, uptimeMs: Date.now() - this.metrics.startedAt }
	}

	onModuleDestroy() { this.store.clear() }
}
