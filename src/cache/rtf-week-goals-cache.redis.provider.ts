 ...
 */
@Injectable()
export class RedisRtfWeekGoalsCacheProvider
  implements IRtfWeekGoalsCacheAsync, OnModuleDestroy
{
  ...
  async invalidateRoutine(routineId: string): Promise<void> {
    ...
    try {
      ...
      do {
        ...
        const res: any = await this.client!.scan(cursor, {
          MATCH: pattern,
          COUNT: 100,
        });
      ...
      if (total > 0)
        this.logger.log(
          `Invalidated ${total} Redis week-goal keys for routine ${routineId}`,
        );
      if (total > 0) this.metrics.invalidations++;
    } catch (err) {
      this.logger.error(`invalidateRoutine failed routineId=${routineId}`, err);
    }
  }
