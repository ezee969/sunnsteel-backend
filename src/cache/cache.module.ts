...
import { AsyncRtfWeekGoalsCacheAdapter, RTF_WEEK_GOALS_CACHE } from './rtf-week-goals-cache.async'
...
function buildProviders() {
	if (useRedis) {
		...
		return [
			...
			...(layeredEnabled ? [
				{ provide: LayeredRtfWeekGoalsCacheProvider, useFactory: (r: RedisRtfWeekGoalsCacheProvider) => new LayeredRtfWeekGoalsCacheProvider(r), inject: [RedisRtfWeekGoalsCacheProvider] },
				{ provide: RTF_WEEK_GOALS_CACHE, useExisting: LayeredRtfWeekGoalsCacheProvider }
			] : [
				{ provide: RTF_WEEK_GOALS_CACHE, useExisting: RedisRtfWeekGoalsCacheProvider }
			])
		]
	}
	return [
		InMemoryRtfWeekGoalsCacheProvider,
		{ provide: AsyncRtfWeekGoalsCacheAdapter, useFactory: (m: InMemoryRtfWeekGoalsCacheProvider) => new AsyncRtfWeekGoalsCacheAdapter(m), inject: [InMemoryRtfWeekGoalsCacheProvider] },
		{ provide: RTF_WEEK_GOALS_CACHE, useExisting: AsyncRtfWeekGoalsCacheAdapter }
	]
}
