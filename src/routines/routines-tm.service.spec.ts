...
} from './exceptions/routine-exceptions';
...
describe('RoutinesService - TM Adjustments', () => {
  ...
  describe('createTmAdjustment', () => {
    ...
    it('should create TM adjustment successfully', async () => {
      ...
      jest
        .spyOn(databaseService.routine, 'findFirst')
    ...
    it('should reject exercise not in routine', async () => {
      ...
      jest
        .spyOn(databaseService.routine, 'findFirst')
        .mockResolvedValue(routineWithoutExercise);
  ...
  describe('getTmAdjustmentSummary', () => {
    it('should return summary statistics', async () => {
      ...
      jest
        ...
        .mockResolvedValue(mockRoutine);
      jest
        .spyOn((databaseService as any).tmAdjustment, 'groupBy')
        .mockResolvedValue(mockSummary as any);
      jest
        .spyOn(databaseService.exercise, 'findMany')
        .mockResolvedValue(mockExercises as any);

      const result = await service.getTmAdjustmentSummary(
        mockUser.id,
        mockRoutine.id,
      );
