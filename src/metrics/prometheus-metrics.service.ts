...
export class PrometheusMetricsService {
  ...
  snapshot() {
    ...
    if (cache.driver === 'layered') {
      ...
      if (
        typeof cache.l2Contribution === 'number' &&
        typeof cache.hits === 'number' &&
        typeof cache.gets === 'number'
      ) {
    ...
    if (w.stampedeWaits > this.lastStampedeWaits)
      this.stampedeWaits.inc(w.stampedeWaits - this.lastStampedeWaits);
    if (w.stampedeBypass > this.lastStampedeBypass)
      this.stampedeBypass.inc(w.stampedeBypass - this.lastStampedeBypass);
    ...
    if (adj > this.lastTmAdjustments)
      this.tmAdjustments.inc(adj - this.lastTmAdjustments);
    if (gr > this.lastTmGuardrailRejects)
      this.tmGuardrailRejects.inc(gr - this.lastTmGuardrailRejects);
    if (ex > this.lastTmUnknownExerciseRejects)
      this.tmUnknownExerciseRejects.inc(ex - this.lastTmUnknownExerciseRejects);
    if (ow > this.lastTmOwnershipRejects)
      this.tmOwnershipRejects.inc(ow - this.lastTmOwnershipRejects);
